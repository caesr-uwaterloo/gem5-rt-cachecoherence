...
void Intel8254Timer::serialize(const string &base, ostream &os) {
...
void Intel8254Timer::unserialize(const string &base, Checkpoint *cp,
...
Intel8254Timer::Counter::Counter(Intel8254Timer *p, const string &name,
                                 unsigned int _num)
    : _name(name), num(_num), event(this), running(false), initial_count(0),
      latched_count(0), period(0), mode(0), output_high(false), latch_on(false),
      read_byte(LSB), write_byte(LSB), parent(p) {
...
bool Intel8254Timer::Counter::outputHigh() { return output_high; }

void Intel8254Timer::Counter::serialize(const string &base, ostream &os) {
...
void Intel8254Timer::Counter::unserialize(const string &base, Checkpoint *cp,
                                          const string &section) {
...
void Intel8254Timer::Counter::CounterEvent::process() {
  switch (counter->mode) {
  case InitTc:
...
const char *Intel8254Timer::Counter::CounterEvent::description() const {
